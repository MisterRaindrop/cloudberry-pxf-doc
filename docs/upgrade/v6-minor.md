...
## Step 2: Installing the New PXF 6.x {#pxfinst}

Install PXF 6.x and identify and note the new PXF version number.


## Step 3: Completing the Upgrade to a Newer PXF 6.x {#pxfup}
...
1. PXF 6.x includes a new version of the `pxf` extension. Register the extension files with Greenplum Database (see [pxf cluster register](../reference/pxf-cluster-cli.md)). `$GPHOME` must be set when you run this command:
...
1. **If you are upgrading <i>from</i> PXF version 6.5.x or earlier <i>to</i> PXF version 6.6.0 or later**, and you want to change the value of the new `pxf.parquet.write.decimal.overflow` property from the default of `round`, add the following to the `pxf-site.xml` file for your PXF server:
    ...
    Refer to [Understanding Overflow Conditions When Writing Numeric Data](../connectors/hdfs/parquet.md#overflow) for more information about how PXF uses this property to direct its actions when it encounters a numeric overflow while writing to a Parquet file.
...
1. **If you are upgrading <i>from</i> PXF version 6.6.x or earlier <i>to</i> PXF version 6.7.0 or later**, and you want to change the value of the new `pxf.orc.write.decimal.overflow` property from the default of `round`, add the following to the `pxf-site.xml` file for your PXF server:
    ...
    Refer to [Understanding Overflow Conditions When Writing Numeric Data](../connectors/hdfs/orc.md#overflow) for more information about how PXF uses this property to direct its actions when it encounters a numeric overflow while writing to an ORC file.

1. **If you are upgrading <i>from</i> PXF version 6.6.x or earlier <i>to</i> PXF version 6.7.0 or later**, and you want to retain the previous PXF listen address (`0.0.0.0`), change the value of the `server.address` `pxf-application.properties` property as described in [Configuring the Listen Address](../administration/host-port.md#listen_address).
...
1. Start PXF on each Greenplum host as described in [Starting PXF](../administration/start-stop-restart.md#start_pxf):
