...
## Step 1: Performing the PXF Pre-Upgrade Actions {#pxfpre}
...
1. Stop PXF on each Greenplum host as described in [Stopping PXF](../administration/start-stop-restart.md#stop_pxf).

## Step 2: Installing PXF 6.x {#pxfinst}

1. Install PXF 6.x and identify and note the new PXF version number.

1. Check out the new installation layout in [About the PXF Installation and Configuration Directories](../administration/directory-structure.md).

## Step 3: Completing the Upgrade to PXF 6.x {#pxfup}
...
1. (Optional, Advanced) If you want to relocate `$PXF_BASE` outside of `<PXF_INSTALL_DIR>`, perform the procedure described in [Relocating $PXF_BASE](../administration/directory-structure.md#movebase).

1. Auto-migrate your PXF 5.x configuration to PXF 6.x `$PXF_BASE`:

    1. Recall your PXF 5.x `$PXF_CONF` setting.
    2. Run the `migrate` command (see [pxf cluster migrate](../reference/pxf-cluster-cli.md)). You must provide `PXF_CONF`. If you relocated `$PXF_BASE`, provide that setting as well.
...
4. **If you are upgrading from PXF version 5.15.x or earlier**:
    ...
    4. Filter pushdown is activated by default for queries on external tables that specify the `Hive`, `HiveRC`, or `HiveORC` profiles (named `hive`, `hive:rc`, and `hive:orc` in PXF version 6.x). If you have previously created an external table that specifies one of these profiles and queries are failing with PXF v5.16+, you can deactivate filter pushdown at the external table-level or at the server level:
        ...
        2. (Server) If you do not want to recreate the external table, you can deactivate filter pushdown *for all* `Hive*` (named as described [here](../connectors/hdfs/overview.md#hadoop_connectors) in PXF version 6.x) *profile queries using the server* by setting the `pxf.ppd.hive` property in the `pxf-site.xml` file to `false`:
...
1. Register the PXF 6.x extension files with Greenplum Database (see [pxf cluster register](../reference/pxf-cluster-cli.md)). `$GPHOME` must be set when you run this command.
...
1. Ensure that you no longer reference previously-deprecated features that were removed in PXF 6.0:
    ...
    | -------------------|-------------|
    | Hadoop profile names | `hdfs:<profile>` as noted [here](../connectors/hdfs/overview.md#hadoop_connectors) |
    | `jdbc.user.impersonation` property | `pxf.service.user.impersonation` property in the [jdbc&#8209;site.xml](../connectors/jdbc/configuration.md#jdbcimpers) server configuration file |
    | `PXF_KEYTAB` configuration property | `pxf.service.kerberos.keytab` property in the [pxf&#8209;site.xml](../administration/server-configuration.md#pxf-site) server configuration file |
    | `PXF_PRINCIPAL` configuration property | `pxf.service.kerberos.principal` property in the [pxf&#8209;site.xml](../administration/server-configuration.md#pxf-site) server configuration file |
    | `PXF_USER_IMPERSONATION` configuration property | `pxf.service.user.impersonation` property in the [pxf&#8209;site.xml](../administration/server-configuration.md#pxf-site) server configuration file |
