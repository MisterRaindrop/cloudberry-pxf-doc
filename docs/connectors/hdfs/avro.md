...
Ensure that you have met the PXF Hadoop [Prerequisites](./overview.md#hadoop_prereq) before you attempt to read data from HDFS.
...
### Data Type Mapping {#profile_hdfsavrodatamap}
...
### Read Mapping {#datatype_map_read }
...
### Write Mapping {#datatype_map_Write}
...
### Avro Schemas and Data {#topic_tr3_dpg_ts__section_m2p_ztg_ts}
...
| DISTRIBUTED BY | If you want to load data from an existing Greenplum Database table into the writable external table, consider specifying the same distribution policy or `<column_name>` on both tables. Doing so will avoid extra motion of data between segments on the load operation. Use the `EXPLAIN` command to analyze the query execution plan and verify that the distribution policy minimizes data movement between segments. |
...
| Keyword  | Value |
|-------|-------------------------------------|
| \<foreign_server\>    | The named server configuration that PXF uses to access the data. You can override credentials in `CREATE SERVER` statement as described in [Overriding the S3 Server Configuration for External Tables](../object-stores/s3-select.md#s3_override_fdw) |
...
| \<custom&#8209;option\>=\<value\> | Avro-specific custom options are described [above](./avro.md#customopts). |


## Example: Reading Avro Data {#avro_example}
...
### Create Schema {#topic_tr3_dpg_ts__section_m2p_ztg_ts_99}
...
### Create Avro Data File (JSON) {#topic_tr3_dpgspk_15g_tsdata}
...
### Reading Avro Data {#topic_avro_querydata}
...
## Writing Avro Data {#topic_avro_writedata}
...
### Example: Writing Avro Data {#topic_avrowrite_example}
