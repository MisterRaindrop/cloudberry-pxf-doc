...
### About Parquet Schemas and Data {#schema }
...
## Example {#parquet_write}

This example utilizes the data schema introduced in [Example: Reading Text Data on HDFS](./text.md#profile_text_query) and adds a new column, `item_quantity_per_order`, an array with length equal to `number_of_orders`, that identifies the number of items in each order.
