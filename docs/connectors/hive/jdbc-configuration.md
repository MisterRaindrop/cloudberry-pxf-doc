...
## JDBC Server Configuration {#hive_cfg_server}
...
## Example Configuration Procedure {#hive_cfg_server_proc}
...
8. If the `hive.server2.authentication` property in `hive-site.xml` is set to `NONE`, or the property is not specified, you must set the `jdbc.user` property. The value to which you set the `jdbc.user` property is dependent upon the `hive.server2.enable.doAs` impersonation setting in `hive-site.xml`:
    ...
    2. If required, create a PXF user configuration file as described in [Configuring a PXF User](../../administration/server-configuration.md#usercfg) to manage the password setting.
...
9. If the `hive.server2.authentication` property in `hive-site.xml` is set to `KERBEROS`:
    ...
    2. Ensure that you have configured Kerberos authentication for PXF as described in [Configuring PXF for Secure HDFS](../../administration/kerberos.md), and that you have specified the Kerberos principal and keytab in the `pxf-site.xml` properties as described in the procedure.
