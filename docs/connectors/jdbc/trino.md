...
## Create a Trino Table {#ex_create_trinotbl}
...
## Configure the Trino Connector {#ex_jdbconfig}
...
1. Download the Trino JDBC driver and place it under `$PXF_BASE/lib`.
   If you [relocated $PXF_BASE](../../administration/directory-structure.md#movebase), make sure you use the updated location.
...
1. Create a JDBC server configuration for Trino as described in [Example Configuration Procedure](./configuration.md#cfg_proc), naming the server directory `trino`.
...
## Read from a Trino Table {#ex_readjdbc}
...
## Write to the Trino Table {#ex_writejdbc}
