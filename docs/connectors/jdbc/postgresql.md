...
## Create a PostgreSQL Table {#ex_create_pgtbl}
...
## Configure the JDBC Connector {#ex_jdbconfig}
...
2. Create a JDBC server configuration for PostgreSQL as described in [Example Configuration Procedure](./configuration.md#cfg_proc), naming the server directory `pgsrvcfg`. The `jdbc-site.xml` file contents should look similar to the following (substitute your PostgreSQL host system for `pgserverhost`):
...
## Read from the PostgreSQL Table {#ex_readjdbc}
...
## Write to the PostgreSQL Table {#ex_writejdbc}
