...
- Use the PostgreSQL database `pgtestdb`, user `pxfuser1`, and PXF JDBC connector server configuration `pgsrvcfg` that you created in [Example: Reading From and Writing to a PostgreSQL Database](./postgresql.md).
...
## Create the PostgreSQL Tables and Assign Permissions {#nq_create_2pgtbl}
...
## Configure the Named Query {#nq_jdbconfig}
...
## Read the Query Results {#nq_readjdbc}
