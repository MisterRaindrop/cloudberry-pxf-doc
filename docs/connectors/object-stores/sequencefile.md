...
| \<custom&#8209;option\>=\<value\> | SequenceFile-specific custom options are described in the [PXF HDFS SequenceFile documentation](../hdfs/sequencefile.md#customopts). |
...
Refer to [Example: Writing Binary Data to HDFS](../hdfs/sequencefile.md#write_seqfile_example) in the PXF HDFS SequenceFile documentation for a write/read example. Modifications that you must make to run the example with an object store include:
