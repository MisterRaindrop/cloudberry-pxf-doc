...
## About JDBC Configuration {#about_cfg}
...
## JDBC Driver JAR Registration {#cfg_jar}

PXF is bundled with the `postgresql-42.4.1.jar` JAR file. If you require a different JDBC driver, ensure that you install the JDBC driver JAR file for the external SQL database in the `$PXF_BASE/lib` directory on each Greenplum host. Be sure to install JDBC driver JAR files that are compatible with your JRE version. See [Registering PXF Library Dependencies](../../administration/register-dependencies.md) for additional information.

## JDBC Server Configuration {#cfg_server}

When you configure the PXF JDBC Connector, you add at least one named PXF server configuration for the connector as described in [Configuring PXF Servers](../../administration/server-configuration.md). You can also configure one or more statically-defined queries to run against the remote SQL database.
...
### Connection-Level Properties {#connprop}
...
### Connection Transaction Isolation Property {#conntransiso}
...
### Statement-Level Properties {#stateprop}
...
### Session-Level Properties {#sessprop}
...
### About JDBC Connection Pooling {#jdbcconpool}
...
#### Tuning the Maximum Connection Pool Size {#jdbcconpool_tune}
...
## About Session Authorization {#sessauth}
...
### Session Authorization Considerations for Connection Pooling {#sessauth_conpool}
...
## JDBC Named Query Configuration {#namedquery}
...
### Defining a Named Query {#namedquery_define}
...
#### Referencing a Named Query {#namedquery_ref}
...
Refer to [About Using Named Queries](./overview.md#about_nq) for information about using PXF JDBC named queries.

## Overriding the JDBC Server Configuration {#cfg_override}

You can override the JDBC server configuration by directly specifying certain JDBC properties via custom options in the `CREATE EXTERNAL TABLE` command `LOCATION` clause. Refer to [Overriding the JDBC Server Configuration via DDL](./overview.md#jdbc_override) for additional information.


## Configuring Access to Hive {#cfg_hive}

You can use the JDBC Connector to access Hive. Refer to [Configuring the JDBC Connector for Hive Access](../hive/jdbc-configuration.md) for detailed information on this configuration procedure.


## Example Configuration Procedure {#cfg_proc}
