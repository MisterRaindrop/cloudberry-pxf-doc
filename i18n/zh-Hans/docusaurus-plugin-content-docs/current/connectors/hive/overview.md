...
:::note
When accessing Hive 3, the PXF Hive connector supports using the <code>hive[:*]</code> profiles described below to access Hive 3 external tables only. The Connector does not support using the <code>hive[:*]</code> profiles to access Hive 3 managed (CRUD and insert-only transactional, and temporary) tables. Use the <a href="jdbc_pxf.html">PXF JDBC Connector</a> to access Hive 3 managed tables instead.
:::

## Prerequisites {#prereq}

Before working with Hive table data using PXF, ensure that you have met the PXF Hadoop [Prerequisites](../hdfs/overview.md#hadoop_prereq).

*If you plan to use PXF filter pushdown with Hive integral types*, ensure that the configuration parameter `hive.metastore.integral.jdo.pushdown` exists and is set to `true` in the `hive-site.xml` file in both your Hadoop cluster **and** `$PXF_BASE/servers/default/hive-site.xml`. Refer to [About Updating Hadoop Configuration](../../getting-started/client-install.md#client-cfg-update) for more information.


## Hive Data Formats {#hive_fileformats}
...
## Data Type Mapping {#hive_datatypemap}

The PXF Hive connector supports primitive and complex data types.

### Primitive Data Types {#hive_datatypemap_prim}
...
### Complex Data Types {#hive_datatypemap_complex}
...
## Sample Data Set {#hive_sampledset}
...
## Hive Command Line {#hive_cmdline}
...
### Example: Creating a Hive Table {#hive_cmdline_example}
...
### Determining the HDFS Location of a Hive Table {#hive_cmdline_fileloc}
...
## Querying External Hive Data {#hive_queryextdata}
...
:::note
Because Hive tables can be backed by one or more files and each file can have a unique layout or schema, PXF requires that the column names that you specify when you create the external table match the column names defined for the Hive table. This allows you to:<ul>
...
## Accessing TextFile-Format Hive Tables {#hive_text}

You can use the `hive` and `hive:text` profiles to access Hive table data stored in TextFile format.

### Example: Using the hive Profile {#hive_hive_example}
...
### Example: Using the hive:text Profile {#hive_hivetext_example}
...
## Accessing RCFile-Format Hive Tables  {#hive_hiverc}
...
### Example: Using the hive:rc Profile {#hive_hiverc_example}
...
## Accessing ORC-Format Hive Tables {#hive_orc}
...
### Profiles Supporting the ORC File Format {#orc-profiles}
...
### Example: Using the hive:orc Profile {#hive_hiveorc_example}
...
### Example: Using the Vectorized hive:orc Profile {#hive_hivevectorizedorc_example}
...
## Accessing Parquet-Format Hive Tables {#hive_parquet}
...
```
## Accessing Avro-Format Hive Tables {#hive_avro}
...
## Working with Complex Data Types {#hive_complex}

### Example: Using the hive Profile with Complex Data Types {#hive_complex_hive}
...
### Example: Using the hive:orc Profile with Complex Data Types {#hive_complex_hiveorc}
...
## Partition Pruning {#partitionfiltering}
...
PXF filter pushdown is enabled by default. You configure PXF filter pushdown as described in [About Filter Pushdown](../../advanced/filter-pushdown.md).


### Example: Using the hive Profile to Access Partitioned Homogenous Data {#hive_homog_part}
...
### Example: Using the hive Profile to Access Partitioned Heterogeneous Data {#hive_heter_part}
...
## Using PXF with Hive Default Partitions {#default_part}
