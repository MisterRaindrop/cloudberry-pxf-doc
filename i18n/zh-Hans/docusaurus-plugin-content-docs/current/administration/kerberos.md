...
You configure these setting for a Hadoop PXF server via the `pxf-site.xml` configuration file. Refer to [About the pxf-site.xml Configuration File](./server-configuration.md#pxf-site) for more information about the configuration properties in this file.

**Note:** PXF supports simultaneous access to multiple Kerberos-secured Hadoop clusters.

## About Kerberos Constrained Delegation {#kcd}
...
- Configured and started PXF as described in [Configuring PXF](../getting-started/install-configure.md).
...
## Use Cases and Configuration Scenarios {#scenarios}
...
### Accessing Hadoop as the Greenplum User {#principal_proxy}
...
![Accessing Hadoop as the Greenplum User Proxied by the Kerberos Principal](/img/graphics/impersonation_cases/impersonation-case-1-kerberos.png "Accessing Hadoop as the Greenplum User Proxied by the Kerberos Principal")
...
|----------------|-------------------|--------------------|-----------|
|  true          | Greenplum user | Perform the [Configuration Procedure](#procedure) in this topic. | Set the Kerberos principal as the Hadoop proxy user as described in [Configure Hadoop Proxying](./user-impersonation.md#hadoop). |
...
![Accessing Hadoop using Kerberos Constrained Delegation](/img/graphics/impersonation_cases/impersonation-case-kcd.png "Accessing Hadoop using Kerberos Constrained Delegation")
...
### Accessing Hadoop as the Kerberos Principal {#principal_hadoop}
...
![Accessing Hadoop as the Kerberos Principal](/img/graphics/impersonation_cases/impersonation-case-2-kerberos.png "Accessing Hadoop as the Kerberos Principal")

The following table identifies the impersonation and service user settings, and the PXF and Hadoop configuration required for this use case:

| Impersonation  | Service&nbsp;User |  PXF Configuration | Hadoop&nbsp;Configuration&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; |
|----------------|-------------------|--------------------|-----------|
|  false         | Identity of the Kerberos principal | Perform the [Configuration Procedure](#procedure.html) in this topic, and then turn off user impersonation as described in [Configure PXF User Impersonation](./user-impersonation.md#pxf_cfg_impers). | None required. |

### Accessing Hadoop as a &lt;custom> User {#custom_hadoop}
...
![Accessing Hadoop as a custom User](/img/graphics/impersonation_cases/impersonation-case-3-kerberos.png "Accessing Hadoop as a custom User")

The following table identifies the impersonation and service user settings, and the PXF and Hadoop configuration required for this use case:

| Impersonation  | Service&nbsp;User |  PXF Configuration | Hadoop&nbsp;Configuration&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; |
|----------------|-------------------|--------------------|-----------|
|  false         |  \<custom>        | Perform the [Configuration Procedure](#procedure) in this topic, turn off user impersonation as described in [Configure PXF User Impersonation](./user-impersonation.md#pxf_cfg_impers), and [Configure the Hadoop User](./user-impersonation.md#pxf_cfg_user) to the \<custom> user name. | Set the Kerberos principal as the Hadoop proxy user as described in [Configure Hadoop Proxying](./user-impersonation.md#hadoop). |
...
| Impersonation  | Service&nbsp;User |  PXF Configuration | AD/IPA&nbsp;Config&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; |
|----------------|-------------------|--------------------|-----------|
|  false          | \<custom\> | Set up the PXF Kerberos principal, keytab files, and related settings in `pxf-site.xml` as described in the [Configuration Procedure](#procedure) in this topic, deactivate impersonation as described in [Configure PXF User Impersonation](./user-impersonation.md#pxf_cfg_impers), [Activate Kerberos Constrained Delegation](#enable_kcd), and [Configure the Hadoop User](./user-impersonation.md#pxf_cfg_user) to the \<custom> user name.  | Configure AD or IPA to provide the PXF Kerberos principal with the delegation rights for the \<custom> user name. |
...
### Configuring PXF with a Microsoft Active Directory Kerberos KDC Server {#proc_ad}
...
### Configuring PXF with an MIT Kerberos KDC Server {#proc_mit}
...
### Completing the PXF Configuration {#proc_complete}
...
1. If you want to access Hadoop as the Greenplum Database user:
    ...
    1. Activate user impersonation as described in [Configure PXF User Impersonation](./user-impersonation.md#pxf_cfg_impers) (this is the default setting).
    ...
    2. *If you did not activate Kerberos constrained delegation*, configure Hadoop proxying for the *primary* component of the Kerberos principal as described in [Configure Hadoop Proxying](./user-impersonation.md#hadoop). For example, if your principal is `gpadmin/_HOST@EXAMPLE.COM`, configure proxying for the Hadoop user `gpadmin`.

1. If you want to access Hadoop using the identity of the Kerberos principal, deactivate user impersonation as described in [Configure PXF User Impersonation](./user-impersonation.md#pxf_cfg_impers).

1. If you want to access Hadoop as a custom user:

    1. Deactivate user impersonation as described in [Configure PXF User Impersonation](./user-impersonation.md#pxf_cfg_impers).
    2. Configure the custom user name as described in [Configure the Hadoop User](./user-impersonation.md#pxf_cfg_user).
...
### Activating Kerberos Constrained Delegation {#enable_kcd}
