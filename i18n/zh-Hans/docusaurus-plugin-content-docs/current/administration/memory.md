...
## Increasing the JVM Memory for PXF {#pxf-heapcfg}
...
5. Restart PXF on each Greenplum Database host as described in [Restarting PXF](./start-stop-restart.md#restart_pxf).


## Configuring Out of Memory Condition Actions {#pxf-cfgoom}
...
### Auto-Terminating the PXF Server {#pxf-cfgoom-autoterm}
...
**When the PXF Service on a host is shut down in this manner, you must explicitly restart the PXF Service on the host.** See the [pxf](../reference/pxf-cli.md) reference page for more information on the `pxf start` command.
...
### Dumping the Java Heap {#pxf-cfgoom-heapdump}
...
7. Restart PXF on each Greenplum Database host as described in [Restarting PXF](./start-stop-restart.md#restart_pxf).


## Another Option for Resource-Constrained PXF Segment Hosts {#pxf-threadcfg}
